      ...
      <div className="space-y-4">
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., Buddy has been less active since yesterday..."
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700">Upload Pictures/Videos</label>
                ...
                {attachments.map((file, index) => (
                    <div key={index} className="flex items-center justify-between p-2 bg-gray-100 rounded-md">
                        <div className="flex items-center space-x-2 truncate">
                            {file.type === 'image' && <img src={file.url} alt={file.name} className="w-8 h-8 object-cover rounded"/>}
                            <span className="text-sm text-gray-700 truncate">{file.name}</span>
                        </div>
                        <button onClick={() => handleRemoveAttachment(index)} className="text-red-500 hover:text-red-700 text-lg">&times;</button>
                    </div>
                ))}
            </div>
          )}
